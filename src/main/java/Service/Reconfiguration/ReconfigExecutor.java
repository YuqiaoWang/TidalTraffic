...
//        for(SimpleEdge currentEdge : area.edges) {
            ...
            try{
                currentEdge.predictedEdgeTraffic =
                        transClient.client.getEdgePredictedData(
                                Tools.inputEdgeDataFormatTrans(currentEdge.nowIntervalEdgeTraffic));
            }catch (Exception e) {
                e.printStackTrace();
            }
...
//            if((double)currentEdge.numberOfOccupatedWavelength / currentEdge.numberOfWavelenth > Tools.DEFAULTTHRESHOLD) {
    ...
    public static void leave(Service service, GraphPath<Vertex, SimpleEdge> originPath,
        ...
        System.out.println("[service " + service.serviceId + "] 已释放原占有资源");
    }
}
