...
layerparameters1 = add_layer(xs, 31, 40, activation_function=tf.nn.relu, weight_name='weights1_area3', bias_name='biases1_area3')
l1 = layerparameters1[0]
Weights1 = layerparameters1[1]
biases1 = layerparameters1[2]

# 隐藏层2
layerparameters2 = add_layer(l1, 40, 10, activation_function = tf.nn.sigmoid, weight_name='weights2_area3', bias_name='biases2_area3')
l2 = layerparameters2[0]
Weights2 = layerparameters2[1]
biases2 = layerparameters2[2]

# add output layer 输入值是隐藏层l1,在预测层输出1个结果
layerparameters3 = add_layer(l2, 10, 16, activation_function=None, weight_name='weights3_area3', bias_name='biases3_area3')
...
# training train_step 和 loss 都是由 placeholder 定义的运算，所以这里要用 feed 传入参数
    ...
    if i % 5000 == 0:
        ...
        saver.save(sess, 'model_save/100erlang/model_area3.ckpt',global_step=i)
...
out_workbook.save('data/100erlang/area3_validation.xls')
print('验证集数据已保存')
